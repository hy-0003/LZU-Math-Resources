...
#include<C:\Users\86182\Desktop\C++\ch5\5.31.cpp>
using namespace std;
...
int main()
{
    ...
    for(int i=0;i<sum;++i)
    {
        cout<<"请输入第"<<i+1<<"个学生的姓名，年龄，各科分数（若无该科请输入成绩0）"<<endl;
        cin>>k[i].name>>k[i].age;
        for(int j=0;j<7;++j)
        {
            cin>>k[i].score[j];
        }
    ...
    i.couttnum(k,sum);
    i.couttave(k,sum);
    cout<<"请输入要查询科目的课程号"<<endl;
    cin>>_objectnum;
    i.coutt(k,sum,_objectnum);
    return 0;
}
